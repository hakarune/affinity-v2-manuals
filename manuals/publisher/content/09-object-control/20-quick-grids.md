...
While drawing a shape, picture frame or text frame, you can tell Affinity to repeat the object in a grid and control the spacing between the grid's columns and rows.

For example, you might want to draw:

- a repeating geometric pattern.
- a series of bars on a chart.
- a grid of picture frames, such as for a contact sheet or image gallery.
- table-like cells that can later be independently transformed and styled, such as for a wall planner.

![An example geometric pattern.](../../assets/shared/gridDiamonds.png)
...
> **Tip:** After drawing a grid, you can use [power duplicate](04-duplicating-objects.md) to add more columns and rows at a later time.
...
> **Note:** The change in spacing when performing steps 4 and 5 is determined by the values of **Nudge Distance** and **Modifier Nudge Distance**, respectively, in **Tools** settings (or preferences).
