...
- ![Copy export setup to clipboard](../../assets/shared/ui/inv-exportcopyconfig.png) **Copy export setup to clipboard**—copies a selected slice's export setup (including multiple export formats) to clipboard. If you select a graphic-specific export format within the setup, the button can be used to copy that export format to another slice's export setup.
- ![Replace export setup from clipboard](../../assets/shared/ui/inv-exportpaste.png) **Replace export setup from clipboard**—pastes a copied slice's export setup from the clipboard to another selected 'target' slice. A copied export format can be pasted too.
- ![Add Export setup from clipboard](../../assets/shared/ui/inv-exportpaste-add.png) **Add Export format from clipboard**—pastes a copied export format selected from a slice's export setup and adds it to another slice's export setup. You can copy (and replace) an export format from Panel Preferences.
- ![Delete](../../assets/shared/ui/trash_can.png) Delete—removes the panel item and corresponding export area from the page. The Page panel item cannot be deleted.
...
- ![Page](../../assets/shared/ui/spread.png) Page—indicates that the export area is the entire page.
- ![Slice](../../assets/shared/ui/Slice_Set.png) Slice—indicates an export area drawn with the Slice Tool.
- ![Slice from Layer](../../assets/shared/ui/icon-node.png) Slice (from item)—indicates an export area created from a layer, group or object.
...
- ![Active](../../assets/shared/ui/visibility_on.png) Active/Inactive—when enabled, the area is exported when **Export Slices (*n*)** is clicked at the bottom of the panel.
- ![Export](../../assets/shared/ui/exported.png) Export—exports the area represented by the panel item.
...
> **Tip:** You can copy and replace export set up options from one slice to another using **Copy/Replace** found on the top of the panel. You can also create your own export set up presets from Panel Preferences ![Panel Preferences](../../assets/shared/ui/moremenuicon.png).
