...
The **Clone Brush Tool** paints samples from one part of an image onto another. It's useful for removing defects, general photo retouching and duplicating parts of an image. Like all brush tools, it is pressure sensitive to create natural looking strokes when using a pressure-sensitive device. The tool conveniently remembers last-used settings.

> **Tip:** The **Clone Brush Tool** can clone from other images (called Global Sources) using the [Sources panel](../../33-panels/24-sources-panel.md).
