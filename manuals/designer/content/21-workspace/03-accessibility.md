# Accessibility

Certain settings and preferences in Affinity Designer can be adjusted to help make the workspace more accessible, improving visibility of designs, readability of text, and adjustability of tool handles.

## Customising the UI appearance

From the app's settings, you can change the UI style, as well as the UI background, artboard background grey level or UI gamma level, adjusting the contrast to improve visibility.

![Contrast before](../../assets/shared/contrast_before.jpg)
![Contrast after](../../assets/shared/contrast_after.jpg)

**To change the UI from dark to light (or vice versa):**

1. Choose **Affinity Designer**>**Settings** (or >**Preferences**).
2. Choose **Edit**>**Settings** (or >**Preferences**).
3. Click the **User Interface** label.
4. For **UI Style**, choose either Dark or Light.

**To change the UI background, Artboard background or Gamma level:**

1. Choose **Affinity Designer**>**Settings** (or >**Preferences**).
2. Choose **Edit**>**Settings** (or >**Preferences**).
3. Click the **User Interface** label.
4. Drag the sliders to set the **Background Grey level**, **Artboard Background Grey level** and/or **UI Gamma** level.

**macOS:**

## Customising the UI font size

The UI font size can be increased via the app's settings to improve readability of text throughout the app.

![Font size before](../../assets/shared/fontsize_before.jpg)
![Font size after](../../assets/shared/fontsize_after.jpg)

**To change the UI font size:**

1. Choose **Affinity Designer**>**Settings** (or >**Preferences**).
2. Choose **Edit**>**Settings** (or >**Preferences**).
3. Click the **User Interface** label.
4. For **UI Font Size**, choose either Default or Large.

## Customising tool handle sizes

Tool handle sizes can be increased and decreased via the app's settings, making them more visible and easier to select.

![Tool handle size before](../../assets/shared/toolhandle_before.jpg)
![Tool handle size after](../../assets/shared/toolhandle_after.jpg)

**To adjust tool handle sizes:**

1. Choose **Affinity Designer**>**Settings** (or >**Preferences**).
2. Choose **Edit**>**Settings** (or >**Preferences**).
3. Click the **Tools** label.
4. Select your preferred **Tool Handle Size** setting.

#### SEE ALSO:

- [Customising the workspace](customise/02-workspace.md)
- [Customising the Toolbar](customise/04-toolbar.md)
