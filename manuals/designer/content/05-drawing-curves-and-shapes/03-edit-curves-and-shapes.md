...
- The ![Node Tool](../../assets/shared/ui/node_tool.png)
...
- `Cmd`-click a segment to delete it, creating two separate curves within one curve object.

> **Note:** ### Modifier keys
>
>
> When using the Node Tool, the following modifier keys can be used to speed up the workflow:
>
>
...
> - The `Ctrl`  keeps both control handles locked in position while moving a node.
> - Pressing left- and right-mouse buttons keeps both control handles locked in position while moving the node.
>
>
...
- ![Split Curve](../../assets/shared/ui/split_curve_at_selected_node.png)
...
- Select the node and press the `Backspace` .

> **Note:** **macOS:**
>
> The curve will automatically reshape because of the deleted node, but you can retain the curve's original geometry by pressing `Alt`+`Backspace`  instead. This needs to be a node that could be removed with the curve still remaining similar.
>
>
> **Windows:**
>
> The curve will automatically reshape because of the deleted node, but you can retain the curve's original geometry by pressing `Cmd`+`Alt`+`Backspace`  instead. This needs to be a node that could be removed with the curve still remaining similar.

**To close curves:**

Do one of the following:

- `Click`-click the end node and select **Close Curve**.
- ![Close Curve](../../assets/shared/ui/action_close_curve.png)

   Select the curve and click **Close Curve** on the context toolbar.
...
- `Click`-click the node at the point at which you want the curve to break and select **Break Curve**.
- ![Break Curve](../../assets/shared/ui/action_break_curve.png)

   Click **Break Curve** on the context toolbar.

**![Join Curves](../../assets/shared/ui/action_join_curves.png)
...
- `Click`-click any node and select **Reverse Curve**.
- ![Reverse Curve](../../assets/shared/ui/action_reverse_curves.png)

   Select any node and then **Reverse Curve** on the context toolbar.

The start node will be toggled from clockwise to anticlockwise. With a closed curve selected, this direction can be viewed using the **Show Orientation** checkbox. With an open curve selected, the direction of the curve is swapped, ready for further drawing from the opposite end of the curve.

**To convert nodes to a different type:**

Do one of the following:

- `Cmd` + `Click`-click the selected node(s) and select **Convert to Sharp**, **Convert to Smooth** or **Convert to Smart**.
- `Click`-click the selected node(s) and select **Convert to Sharp**, **Convert to Smooth** or **Convert to Smart**.
- ![Sharp Corner](../../assets/shared/ui/convert_sharp.png)

   ![Smooth Corner](../../assets/shared/ui/convert_smooth.png)

   ![Smart Corner](../../assets/shared/ui/convert_smart.png)

   Select the node(s) and click an equivalent **Convert** option on the context toolbar.
