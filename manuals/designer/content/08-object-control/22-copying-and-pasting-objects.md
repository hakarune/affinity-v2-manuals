...
The **Copy items as SVG** preference [(accessed via **Settings** or **Preferences** (**General** section)] copies objects in SVG format in readiness for pasting to external apps.
...
<sup>3</sup> If copied text's formatting includes settings that are incompatible with the target text, the incompatible settings will not be pasted. For example, horizontal centring is ignored when pasting onto a range *within* a left-aligned paragraph.
...
- [Targeting objects](13-targeting-objects.md)
