...
| ![Align handle positions using snapping options](../../assets/shared/ui/snap_curve_handle_position.png)   Align handle positions using snapping options | Snaps control handle using any of the snapping criteria currently set in Global snapping, e.g. to grid, guide, object geometry/node (shown in yellow), key points, etc. | ![Align handle positions using snapping options](../../assets/shared/curve_align_handles.png) |
...
> **Note:** When drawing, you can also snap the moving node to any curve intersection point or node on the same or different curve if the global snapping option **Snap to object geometry** is enabled.
>
...
2. From the context toolbar's **Snap** section, enable combinations of the buttons for different curve snapping behaviour.
