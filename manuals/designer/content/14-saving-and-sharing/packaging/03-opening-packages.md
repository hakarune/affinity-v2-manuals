...
When an .afpackage file is opened, Affinity Designer displays the **Package Document** dialog to inform the recipient of the package's contents, including any font and image issues they may need to resolve.
...
- **Package**—indicates whether a corresponding font file is included in the package. If included, it may be listed as [Restricted](02-creating-packages.md).
- **Install**—when ticked, Affinity Designer will temporarily load the included font file for its exclusive and temporary use.
...
If such a font is included in the package, tick the checkbox in its **Install** column to make it available only to Affinity Designer and only for the current session. This enables the recipient to export and print the document as is.
...
You will not need to locate images listed as Present. When packaging, Affinity Designer updates a document's links to these resources to use the files in the package's Images folder.
...
4. Select **OK**.
5. (Optional) Select **File>Save As**. This creates an .afdesign file to which you can save changes and which you are now editing.
